...
# created by OdzioM
# script filename: libadwaita-tc-gui-9.py
#
############################################
...
class ThemeChanger:
    ...
    def create_widgets(self):
        tk.Label(self.root, text="Change LibAdwaita Theme", font=("Arial", 24)).pack(pady=(20, 10), anchor="w")
        tk.Label(self.root, text="Choose theme to activate:", anchor="w").pack(fill="x")
        theme_menu = tk.OptionMenu(self.root, self.theme_var, *self.themes)
        theme_menu.pack(fill="x")
        tk.Button(self.root, text="&Activate", command=self.apply_theme).pack(fill="x")
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="E&xit", command=self.root.destroy, underline=1, accelerator="Alt+F4")
        ...
        options_menu.add_command(label="S&ettings", command=self.settings_screen, underline=1, accelerator="Alt+O")
        self.root.bind_all("<Alt-o>", lambda e: self.settings_screen())
